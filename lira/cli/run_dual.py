...
class EncoderWrapper:
    def __init__(self, model_path, providers=None):
        ...
        self.streaming = any('cached_' in name or 'cached_len_' in name for name in self.input_names)
    ...
    def run(self, x):
        ...
        if self.streaming:
            self.cache = {o.name.replace('new_cached_', ''): val for o, val in zip(self.session.get_outputs()[1:], outs[1:])}
        return outs[0]
...
class WhisperWrapper:
    ...
    def run(self, features):
        ...
        for _ in range(448):
            input_ids = np.full((1, 448), self.eos_token, dtype=np.int64)  # fill with eos_token
            ...
            next_token = int(np.argmax(logits[0, len(tokens)-1]))  # get logits from actual position
