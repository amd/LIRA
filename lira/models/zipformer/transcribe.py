...
class ZipformerONNX:
    def __init__(self, encoder_path, decoder_path, joiner_path, tokens, device, config_path="config/model_config.json"):
        ...
        self.encoder = EncoderWrapper(encoder_path, providers=providers_config.get("encoder", get_providers(device)))
        self.decoder = DecoderWrapper(decoder_path, providers=providers_config.get("decoder", get_providers(device)))
        self.joiner = JoinerWrapper(joiner_path, providers=providers_config.get("joiner", get_providers(device)))
    ...
    def transcribe(self, audio):
        ...
        transcription = greedy_search(self.encoder, self.decoder, self.joiner, features, tokens, state)
    ...
    def parse_cli(subparsers):
        zipformer_parser = subparsers.add_parser("zipformer", help="Run Zipformer model")
        zipformer_parser.add_argument("--model-dir", help="Path to model directory (local or Hugging Face repo)")
        zipformer_parser.add_argument("--encoder", help="Path to encoder model (overrides model-dir)")
        zipformer_parser.add_argument("--decoder", help="Path to decoder model (overrides model-dir)")
        zipformer_parser.add_argument("--joiner", help="Path to joiner model (overrides model-dir)")
        zipformer_parser.add_argument("--tokens", help="Path to tokens file (overrides model-dir)")
        zipformer_parser.add_argument("--audio", required=True, help="Path to audio file")
        zipformer_parser.add_argument("--device", choices=["cpu", "npu", "igpu"], default="cpu", help="Device to run the model on")
    ...
    def run(args):
        ...
        if not args.model_dir and not (args.encoder and args.decoder and args.joiner and args.tokens):
            raise ValueError("Either --model-dir or all of --encoder, --decoder, --joiner, and --tokens must be provided.")
        ...
        encoder_path = args.encoder if args.encoder else os.path.join(model_dir, "encoder.onnx")
        decoder_path = args.decoder if args.decoder else os.path.join(model_dir, "decoder.onnx")
        joiner_path = args.joiner if args.joiner else os.path.join(model_dir, "joiner.onnx")
        tokens_path = args.tokens if args.tokens else os.path.join(model_dir, "tokens.txt")
