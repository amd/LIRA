...
def main():
    ...
    # 'compare' command
    # Not Implemented
    compare_parser = subparsers.add_parser("compare", help="Run multiple models simultaneously + chain tools")
    compare_parser.add_argument("--models", nargs="+", required=True, help="List of models to compare")
    compare_parser.add_argument("--input", required=True, help="Path to input audio file")

    # 'tool' command
    tool_parser = subparsers.add_parser("tool", help="Run standalone tools or chain them")
    tool_parser.add_argument("--name", required=True, help="Name of the tool to run")
    tool_parser.add_argument("--args", nargs=argparse.REMAINDER, help="Arguments for the tool")
