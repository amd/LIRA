...
def main():
    ...
    parser.add_argument(
        "--model-dir",
        required=True,
        help="Path to local model directory or HF repo name",
    )
    parser.add_argument(
        "--duration", type=int, default=0, help="Duration for mic input (seconds)"
    )
    parser.add_argument(
        "--device", choices=["cpu", "npu"], default="cpu", help="Target device"
    )
    parser.add_argument(
        "--model-type",
        choices=["zipformer", "whisper-base", "whisper-small", "whisper-medium"],
        required=True,
        help="Model type",
    )
    parser.add_argument(
    ...
    providers = get_model_providers(
        args.model_type, args.device, config_path=args.config_path
    )

    if args.model_type == "zipformer":
        encoder = EncoderWrapper(
            str(model_dir / "encoder.onnx"), providers=providers["encoder"]
        )
        decoder = DecoderWrapper(
            str(model_dir / "decoder.onnx"), providers=providers["decoder"]
        )
        joiner = JoinerWrapper(
            str(model_dir / "joiner.onnx"), providers=providers["joiner"]
        )
